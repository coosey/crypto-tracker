...
export default async function handler(
  ...
  const { query } = req;
  const { page = 1 } = query;
  const URL = `${process.env.NEXT_PUBLIC_COINGECKO_API_URL}/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=${page}&price_change_percentage=1h%2C24h%2C7d&locale=en`;
  try {
